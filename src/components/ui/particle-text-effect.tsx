...
const DEFAULT_WORDS = ["HELLO", "DevServer", "Particles", "BY", "YOU"];
...
export function ParticleTextEffect({ words = DEFAULT_WORDS, className }: ParticleTextEffectProps) {
  ...
  useEffect(() => {
      ...
      for (const coordIndex of coordsIndexes) {
        ...
        if (alpha > 0) {
          ...
          if (particleIndex < particles.length) {
            ...
            const randomPos = generateRandomPos(c.width / 2, c.height / 2, (c.width + c.height) / 2);
    ...
    const animate = () => {
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        particle.move();
        particle.draw(cctx, drawAsPoints);
      ...
      if (frameCountRef.current % 240 === 0 && currentWords.length > 0) {
        wordIndexRef.current = (wordIndexRef.current + 1) % currentWords.length;
        nextWord(currentWords[wordIndexRef.current], c);
      }

      animationRef.current = requestAnimationFrame(animate);
    };

    nextWord(wordsRef.current[0] ?? "HELLO", canvas);
    animate();
  ...
  return (
    <div className={className ?? "flex min-h-screen flex-col items-center justify-center bg-black p-4"}>
      <canvas
        ref={canvasRef}
      ...
      <div className="mt-4 max-w-md text-center text-sm text-white">
        ...
        <p className="text-xs text-gray-400">
          Botão direito + arrastar para dispersar partículas · palavras alternam a cada ~4s
        </p>
      </div>
    </div>
  );
