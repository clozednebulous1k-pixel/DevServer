...
export function DicedHeroSection({
  ...
  const ordered = [slides[3], slides[2], slides[1], slides[0]].filter(Boolean);
    ...
    >
      ...
      <style jsx>{`
        ...
        .top-right {
          ...
          --_d: (var(--s) + var(--r));
          mask: calc(100% - var(--_d) - var(--x)) 0 var(--_m), 100% calc(var(--_d) + var(--y)) var(--_m),
            radial-gradient(var(--s) at 100% 0, #0000 99%, #000 calc(100% + 1px)) calc(-1 * var(--r) - var(--x)) calc(var(--r) + var(--y)),
            var(--_g) calc(-1 * var(--_d) - var(--x)) 0, var(--_g) 0 calc(var(--_d) + var(--y));
