...
export function Radar({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("relative flex h-20 w-20 items-center justify-center rounded-full", className)}>
      ...
      >
        <div className="relative z-40 h-px w-full bg-gradient-to-r from-transparent via-sky-600 to-transparent" />
      </div>
      {circles.map((_, idx) => (
        <RadarCircle
          style={{
...
export function RadarIconContainer({
  icon,
  text,
  delay,
}: {
  ...
  return (
    ...
    >
      <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-slate-700 bg-slate-800 shadow-inner">
        {icon ?? (
          <svg className="h-8 w-8 text-slate-600" fill="currentColor" viewBox="0 0 20 20" aria-hidden>
            <path
              fillRule="evenodd"
      ...
      <div className="hidden rounded-md px-2 py-1 md:block">
        <div className="text-center text-xs font-bold text-slate-400">{text ?? "Web Development"}</div>
      </div>
    </motion.div>
  );
}
